...
def admin_dashboard(request):
    ...
    incidentReports_pending = IncidentGeneral.objects.filter(user_report__status = 1)
    today = datetime.today().date()
    incidentReports_today = IncidentGeneral.objects.filter(created_at__date=today)
    incidentReports_approved = IncidentGeneral.objects.filter(user_report__status = 2)
    ...
    if fromdate:    
        incident_general = incident_general.filter(user_report__date__gte=fromdate)
        incident_vehicle = incident_vehicle.filter(incident_general__user_report__date__gte=fromdate)
    if todate:
        incident_general = incident_general.filter(user_report__date__lte=todate)
        incident_vehicle = incident_vehicle.filter(incident_general__user_report__date__gte=fromdate)
    ...
    queryset = incident_general.exclude(severity = None).values('severity').annotate(severity_count=Count('severity'))
    
    data = list(queryset.values_list('severity_count', flat=True))
    labels = list(queryset.values_list('severity', flat=True))
    
    queryset1 = incident_vehicle.exclude(vehicle_type = None).values('vehicle_type').annotate(vehicle_type_count=Count('vehicle_type'))
    
    data1 = list(queryset1.values_list('vehicle_type_count', flat=True))
    labels1 = list(queryset1.values_list('vehicle_type', flat=True))
    
    queryset2 = user_report.exclude(date = None).values('date').annotate(status_count=Count('status'))
    ...
    df = pd.DataFrame(incident_general.values('user_report__latitude', 'user_report__longitude', 'accident_factor'))
    ...
    folium.TileLayer('cartodbdark_matter', attr='cartodbdark_matter').add_to(map2)
    plugins.Fullscreen(position='topright').add_to(map2)
    folium.LayerControl().add_to(map2)

    for id,row in df.iterrows():
        folium.CircleMarker( location=[ row['user_report__latitude'],row['user_report__longitude'] ],radius=2, fill=True).add_to( map2 )
